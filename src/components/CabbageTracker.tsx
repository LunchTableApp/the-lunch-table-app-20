...
export const CabbageTracker = ({
  ...
  return <div className="bg-white dark:bg-black rounded-lg shadow-md p-6 mb-8 animate-fadeIn">
      ...
      <div className="flex justify-center mb-4">
        <div className="relative w-64 h-64 flex items-center justify-center">
          ...
          <svg className="w-full h-full transform -rotate-90">
            <circle cx="128" cy="128" r={radius} className="stroke-gray-200 dark:stroke-gray-700" strokeWidth="12" fill="none" />
            {/* Progress circle */}
            <circle cx="128" cy="128" r={radius} className="stroke-primary transition-all duration-500" strokeWidth="12" fill="none" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" />
          </svg>
          ...
          {isGoalCompleted && <>
              {Array.from({
            length: 12
          }).map((_, i) => <div key={`confetti-${i}`} className="absolute w-2 h-2 bg-primary animate-ping" style={{
      ...
      <div className="text-center">
        {isGoalCompleted ? <p className="text-lg font-bold text-green-500">
            🎉 Congratulations! You've reached your {monthName} goal! 🎉
          </p> : <p className="text-lg font-medium dark:text-white">
